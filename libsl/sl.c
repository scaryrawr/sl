...
typedef wchar_t slchar_t;

void set_locale();
void add_smoke(int y, int x);
void add_man(int y, int x);
int add_C51(int x, slchar_t *namelist[], int cars);
int add_D51(int x, slchar_t *namelist[], int cars);
int add_sl(int x, slchar_t *namelist[], int cars);
...
int add_sl(int x, slchar_t *namelist[], int cars) {
  ...
  static wchar_t *coal[LOGOHEIGHT + 1] = {LCOAL1, LCOAL2, LCOAL3, LCOAL4,
                                          LCOAL5, LCOAL6, DELLN};

  static wchar_t *car[LOGOHEIGHT + 1] = {LCAR1, LCAR2, LCAR3, LCAR4,
                                         LCAR5, LCAR6, DELLN};
  ...
  for (i = 0; i <= LOGOHEIGHT; ++i) {
    ...
    for (j = 0; j < cars; ++j) {
      ...
      my_mvaddstr(y + i + (FLY * j) + py2, x + 42 + (LCARLENGTH - 1) * j,
                  carName);
    }
  }
...
int add_D51(int x, slchar_t *namelist[], int cars) {
  ...
  static wchar_t *coal[D51HEIGHT + 1] = {COAL01, COAL02, COAL03, COAL04,
                                         COAL05, COAL06, COAL07, COAL08,
                                         COAL09, COAL10, COALDEL};

  static wchar_t *car[D51HEIGHT + 1] = {CAR01, CAR02, CAR03,  CAR04,
                                        CAR05, CAR06, CAR07,  CAR08,
                                        CAR09, CAR10, COALDEL};
  ...
  for (i = 0; i <= D51HEIGHT; ++i) {
    ...
    for (j = 0; j < cars; ++j) {
      ...
      my_mvaddstr(y + i + (FLY * (j + 1)) + dy,
                  x + 53 + (CARLENGTH - 3) * (j + 1), carName);
    }
  }
  ...
  if (ACCIDENT == 1) {
    ...
    for (j = 0; j < cars; ++j) {
      ...
      add_man(y + 1 + (FLY * (j + 2)),
              x + D51LENGTH + 5 + ((CARLENGTH - 3) * j));
      add_man(y + 1 + (FLY * (j + 2)),
              x + D51LENGTH + 15 + ((CARLENGTH - 3) * j));
    }
  }
...
int add_C51(int x, slchar_t *namelist[], int cars) {
  ...
  static wchar_t *coal[C51HEIGHT + 1] = {COALDEL, COAL01, COAL02, COAL03,
                                         COAL04,  COAL05, COAL06, COAL07,
                                         COAL08,  COAL09, COAL10, COALDEL};

  static wchar_t *car[C51HEIGHT + 1] = {COALDEL, CAR01, CAR02, CAR03,
                                        CAR04,   CAR05, CAR06, CAR07,
                                        CAR08,   CAR09, CAR10, COALDEL};
  ...
  if (ACCIDENT == 1) {
    ...
    for (j = 0; j < cars; ++j) {
      ...
      add_man(y + 2 + (FLY * (j + 2)),
              x + C51LENGTH + 3 + ((CARLENGTH - 3) * j));
      add_man(y + 2 + (FLY * (j + 2)),
              x + C51LENGTH + 13 + ((CARLENGTH - 3) * j));
    }
  }
...
#define SMOKEPTNS 16
{
  ...
  static wchar_t *Smoke[2][SMOKEPTNS] = {
      {L"(   )", L"(    )", L"(    )", L"(   )", L"(  )", L"(  )", L"( )",
       L"( )", L"()", L"()", L"O", L"O", L"O", L"O", L"O", L" "},
      {L"(@@@)", L"(@@@@)", L"(@@@@)", L"(@@@)", L"(@@)", L"(@@)", L"(@)",
       L"(@)", L"@@", L"@@", L"@", L"@", L"@", L"@", L"@", L" "}};
  static wchar_t *Eraser[SMOKEPTNS] = {L"     ", L"      ", L"      ", L"     ",
                                       L"    ",  L"    ",   L"   ",    L"   ",
                                       L"  ",    L"  ",     L" ",      L" ",
                                       L" ",     L" ",      L" ",      L" "};
