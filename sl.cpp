...
int add_D51(int x);
int add_sl(int x);
void option(std::string_view str);
int my_mvaddstr(int y, int x, std::wstring_view str);
...
int add_sl(int x)
{
    constexpr std::wstring_view sl[LOGOPATTERNS][LOGOHEIGHT + 1] = {{LOGO1, LOGO2, LOGO3, LOGO4, LWHL11, LWHL12, DELLN},
    ...
    constexpr std::wstring_view coal[] = {LCOAL1, LCOAL2, LCOAL3, LCOAL4, LCOAL5, LCOAL6, DELLN};

    constexpr std::wstring_view car[] = {LCAR1, LCAR2, LCAR3, LCAR4, LCAR5, LCAR6, DELLN};
    ...
    for (int i = 0; i <= LOGOHEIGHT; ++i)
    {
        ...
        {
            int pos = LOGOLENGTH + x + (LCARLENGTH - 1) * (j + 1);
            if (pos < 0)
            {
                continue;
            }
            ...
            auto carName{std::vformat(car[i], std::make_wformat_args(namelist[j]))};
...
int add_D51(int x)
{
    constexpr std::wstring_view d51[D51PATTERNS][D51HEIGHT + 1] = {{D51STR1, D51STR2, D51STR3, D51STR4, D51STR5, D51STR6, D51STR7,
    ...
    constexpr std::wstring_view coal[D51HEIGHT + 1] = {COAL01, COAL02, COAL03, COAL04, COAL05,
                                                       COAL06, COAL07, COAL08, COAL09, COAL10, COALDEL};

    constexpr std::wstring_view car[D51HEIGHT + 1] = {CAR01, CAR02, CAR03, CAR04, CAR05,
    ...
    for (int i = 0; i <= D51HEIGHT; ++i)
    {
        ...
        for (int j = 0; j < cars; ++j)
        {
            int pos = D51LENGTH + x + (CARLENGTH - 3) * (j + 1);
            if (pos < 0)
            {
                continue;
            }
...
int add_C51(int x)
{
    constexpr std::wstring_view c51[C51PATTERNS][C51HEIGHT + 1] = {{C51STR1, C51STR2, C51STR3, C51STR4, C51STR5, C51STR6, C51STR7,
    ...
    constexpr std::wstring_view coal[C51HEIGHT + 1] = {COALDEL, COAL01, COAL02, COAL03, COAL04, COAL05,
                                                       COAL06, COAL07, COAL08, COAL09, COAL10, COALDEL};

    constexpr std::wstring_view car[C51HEIGHT + 1] = {COALDEL, CAR01, CAR02, CAR03, CAR04, CAR05,
                                                      CAR06, CAR07, CAR08, CAR09, CAR10, COALDEL};
    ...
    for (int i = 0; i <= C51HEIGHT; ++i)
    {
        ...
        for (int j = 0; j < cars; ++j)
        {
            int pos = C51LENGTH + x + (CARLENGTH - 3) * (j + 1);
            if (pos < 0)
            {
                continue;
            }
...
void add_man(int y, int x)
{
    constexpr std::wstring_view man[2][2] = {{L"", L"(O)"}, {L"Help!", L"\\O/"}};
    for (int i = 0; i < 2; ++i)
    {
...
#define SMOKEPTNS 16
{
    ...
    static int sum = 0;
    constexpr std::wstring_view Smoke[2][SMOKEPTNS] = {{L"(   )", L"(    )", L"(    )", L"(   )", L"(  )",
                                                        L"(  )", L"( )", L"( )", L"()", L"()",
                                                        L"O", L"O", L"O", L"O", L"O",
                                                        L" "},
                                                       {L"(@@@)", L"(@@@@)", L"(@@@@)", L"(@@@)", L"(@@)",
                                                        ...
                                                        L"@", L"@", L"@", L"@", L"@",
                                                        L" "}};
    constexpr std::wstring_view Eraser[SMOKEPTNS] = {L"     ", L"      ", L"      ", L"     ", L"    ",
                                                     L"    ", L"   ", L"   ", L"  ", L"  ",
                                                     L" ", L" ", L" ", L" ", L" ",
                                                     L" "};
